...
const ViewPrescriptionForm = ({ onClose, prescriptionDetails }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const employeeData = await employeeRequest(
          prescriptionDetails.employee
        );
        setEmployee(employeeData);
        const patientData = await patientRequest(prescriptionDetails.patient);
        setPatient(patientData);
        const clinicData = await getClinicByIdRequest(
  ...
  const handleGeneratePDF = async () => {
    ...
    if (content) {
      ...
      try {
        ...
        const visitDatetime =
          dayjs(prescriptionDetails.prescription_date).format("YYYY-MM-DD") ||
          "UnknownDate";
  ...
  return (
    <div>
      <div
        className={`overlay-prescription-view ${isFormOpen ? "active" : ""}`}
      ></div>
      <div className="popup-form-prescription">
        <form className="form-sections-prescription" id="prescription-form">
          ...
          <div className="form-section-info">
            ...
            <div className="medication-items">
              {prescriptionDetails.prescribed_medications.map(
                (medication, index) => (
                  <div key={index} className="medication-item">
                    <div className="medication-info">
                      <div className="medication-row">
                        ...
                        <p className="medication-amount">
                          {medication.medication_amount}
                        </p>
                      </div>
                      <p className="medication-dosage">
                        Dawkowanie: {medication.medication_dosage}
                      </p>
                    </div>
                  </div>
                )
        ...
        <div className="button-container-prescription">
          <button
            ...
            className="form-button-prescription-form"
